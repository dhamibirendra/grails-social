...
import com.ds.social.Person
import com.ds.social.Authority
import com.ds.social.PersonAuthority
import com.ds.social.OAuthID
...
class SpringSecurityOAuthController {
    ...
    def linkAccount = { OAuthLinkAccountCommand command ->
        ...
        if (request.post) {
            boolean linked = command.validate() && Person.withTransaction { status ->
                Person user = Person.findByUsernameAndPassword(
    ...
    def createAccount = { OAuthCreateAccountCommand command ->
        ...
        if (request.post) {
            if (!springSecurityService.loggedIn) {
                ...
                boolean created = command.validate() && Person.withTransaction { status ->
                    Person user = new Person(username: command.username, password: command.password1, enabled: true)
                    user.addToOAuthIDs(provider: oAuthToken.providerName, accessToken: oAuthToken.socialId, user: user)
                    ...
                    for (roleName in config.oauth.registration.roleNames) {
                        PersonAuthority.create user, Authority.findByAuthority(roleName)
                    }
    ...
    protected OAuthToken updateOAuthToken(OAuthToken oAuthToken, Person user) {
...
/*
    private def updateUser(Person user, OAuthToken oAuthToken) {
        if (!user.validate()) {
            return
        }
